 ...
 * (UPI tokens, Shiprocket tokens, buyer addresses) will become unreadable.
 */
function getKey(): Buffer {
    const secret = process.env.DATA_ENCRYPTION_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY;
    if (!secret) {
        throw new Error("DATA_ENCRYPTION_KEY or SUPABASE_SERVICE_ROLE_KEY required for encryption");
    }
    return scryptSync(secret, "zavvy-upi-salt", KEY_LEN);
}
 ...
 * 
 * @param plaintext - Sensitive data to encrypt (e.g., UPI ID)
 ...
 * Use immediately and discard after use (e.g., for generating UPI links).
 * 
 * @param hex - Hex-encoded encrypted string from encrypt()
 ...
 * Examples: 9876543210@paytm, username@ybl, merchant.name@icici
 * 
 * @param upiId - UPI ID to validate
 * @returns true if valid format
 */
export function validateUpiId(upiId: string): boolean {
    // UPI format: alphanumeric + dots/underscores/hyphens @ bank code
