...
export async function GET(
    ...
    try {
        ...
        const { data: seller } = await db.from("sellers").select("id, business_name, upi_id")
...
async function generateLabel(order: any, seller: any): Promise<Uint8Array> {
    ...
    page.drawText(`Order Date: ${dateStr} | Powered by Zavvy`, {
        x: 15, y: 8, size: 7, font: fontRegular, color: gray
    });
